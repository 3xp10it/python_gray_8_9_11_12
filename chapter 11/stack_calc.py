...
for functions in Functions(SegStart(current_address),SegEnd(current_address)):
		...
		if stack_var!="":
			if prev_count!=-1:
				...
				if distance>=var_size_threshold:
					print "function:%s->stack variable:%s(%d bytes)" % (GetFuntionName(function),prev_member,distance)
				else:
